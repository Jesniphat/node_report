...
var code128 = function(code, width, height) {
  barcodeType = code128Detect(code);
  if (barcodeType=='C' && code.length%2==1) code = '0'+code;
  var a = parseBarcode(code,  barcodeType);
  return code2svg(a.join(''), width, height);
}

var code2svg = function(code, width, height) {
  var pos = 10;
  var out = ['M 10 0'];
  var len = code.length;
  var w = 20;
  for (var i = 0; i < len; i++) {
    w += parseInt(code.charAt(i));
  }
  var ratio = 1.0*width/w;
  for (var i = 0; i < len; i+=2) {
    var num = parseInt(code.charAt(i));
    var n = num*ratio;
    out.push('l 0 ' + height + ' l ' + n + ' 0 l 0 -' + height + ' z');
    if (i+2 < len) {
      out.push('m ' + (n+parseInt(code.charAt(i+1)) * ratio)+' 0');
    }
  }
  return out.join(' ');
}

var code128Detect = function(code) {
  if (/^[0-9]+$/.test(code)) return 'C';
  if (/[a-z]/.test(code)) return 'B';
  return 'A';
}

var parseBarcode = function(barcode, barcodeType) {
  var bars = [];
